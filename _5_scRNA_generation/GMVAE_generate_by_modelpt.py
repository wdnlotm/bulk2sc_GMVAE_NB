...
def nb_gen(mean, disp):
    success_prob=(disp)/(mean+disp)
    gen_value=np.random.negative_binomial(disp, success_prob)
    return gen_value
...
def generate_from_pt_cellcounts(saved_model:str, cell_counts:list, factor=1, suffix=''):
    ...
    for ii in range(gmvae.args.K):
        ...
        gen_mean, gen_disp = gmvae.decoder(gen_z)
    ...
    generated_matrix = nb_gen(generated_mean_np,  generated_disp_np)
