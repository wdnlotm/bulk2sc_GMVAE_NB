...
from GMVAE_scrna_nb import GMVAE_NB
from GMVAE_losses_nb import *
from GMVAE_utils_nb import train as gmvae_train
from GMVAE_utils_nb import test as gmvae_test

parser = argparse.ArgumentParser(description='Gaussian Mixture VAE with NB')
...
parser.add_argument('-mn','--model-name', type=str, default="GMVAE_w_nb", help='model description')
...
gmvae = GMVAE_NB(args).to(device)
print(gmvae)
print(vars(args))

with open("args_save_nb.pickle", "wb") as output_file:
...
if not (args.restart):
    ...
    for epoch in progress_bar:
        # train the network
        total_loss_epoch, KLD_gaussian_epoch, KLD_pi_epoch, nb_loss_epoch, accuracy \
        ...
        tf_writer.add_scalar("reconst_loss/train", nb_loss_epoch, epoch)
        tf_writer.add_scalar("accuracy/train", accuracy, epoch)

        if epoch % args.testfreq == 0:
            total_loss_test, KLD_gaussian_test, KLD_pi_test, nb_loss_test, accuracy_test \
            ...
            tf_writer.add_scalar("reconst_loss/test", nb_loss_test, epoch)
